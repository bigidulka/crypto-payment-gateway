    ...
    async def _collect_chain_balances(
        ...
        async with get_session_context() as session:
            ...
            try:
                ...
                token_balances_task = adapter.get_balances_batch(addresses, token_contracts)
                ...
                try:
                    ...
                    if plan.needs_gas_funding:
                        try:
                            ...
                            logger.debug(f"[{chain}] Gas sent to {wallet.address[:10]}...: {tx_hash}")
                            # Ждём подтверждения газа
                            await asyncio.sleep(3)
                        except Exception as e:
                            logger.error(f"[{chain}] Gas funding failed for {wallet.address[:10]}...: {e}")
                            return False
                    
                    # 2. Выполняем sweep
                    privkey = decrypt_private_key(wallet.encrypted_privkey, self.encryption_key)
                    ...
                    logger.error(f"[{chain}] Sweep failed for {plan.wallet.address[:10]}...: {e}")
                    return False
