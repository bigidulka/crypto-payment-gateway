...
class ResilientLogFetcher:
    ...
    OR_TOPICS_LIMIT = 50  # Public RPC-safe address count per OR-topics request
    MAX_SINGLE_ADDRESS_FALLBACK = 100  # Avoid 500+ per-address RPC storms
    ...
    async def _fetch_chunked_or_topics(
        self,
        endpoint: RpcEndpointState,
        from_block: int,
        to_block: int,
        ...
        """Fetch logs using multiple small OR-topics requests."""
        all_logs: list[dict] = []

        for i in range(0, len(to_addresses), self.OR_TOPICS_LIMIT):
            chunk = to_addresses[i : i + self.OR_TOPICS_LIMIT]
            chunk_logs = await self._fetch_with_or_topics(
                endpoint,
                from_block,
                to_block,
                chunk,
            ...
            # Public RPCs rate-limit bursty eth_getLogs. Small pacing keeps
            # persistent scans complete instead of tripping 429/circuit breakers.
    ...
    async def fetch_transfer_logs(
        ...
        # === PHASE 1: Try chunked OR Topics on every configured endpoint ===

        for endpoint in self.endpoints:
            if not endpoint.circuit_or_topics.can_execute():
                continue

            if not endpoint.or_topics_tested:
                await self._test_or_topics_support(endpoint)

            if not endpoint.supports_or_topics:
                continue

            try:
                start = time.time()
                logs = await self._fetch_chunked_or_topics(
                    endpoint, from_block, to_block, to_addresses, token_contracts
                )
                latency = (time.time() - start) * 1000

                logger.debug(
                    f"[{self.chain}] Chunked OR Topics success: "
                    f"{len(logs)} logs in {latency:.0f}ms via {endpoint.url}"
                )
                ...
                last_error = e
                logger.warning(
                    f"[{self.chain}] Chunked OR Topics failed via {endpoint.url}: {e}"
                )

        if len(to_addresses) > self.MAX_SINGLE_ADDRESS_FALLBACK:
            logger.error(
                f"[{self.chain}] OR Topics failed for {len(to_addresses)} addresses; "
                f"skipping per-address fallback to avoid RPC storm. Last error: {last_error}"
            )
            return TransferLogResult(
                logs=[],
                method_used=FetchMethod.OR_TOPICS,
