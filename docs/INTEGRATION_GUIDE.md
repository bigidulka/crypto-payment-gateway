# 🔌 Руководство по интеграции Arbitron Payment Gateway
...
ARBITRON_API_KEY=sk_live_abc123xyz789...
```
...
async def get_deposit_address(user_id: str, chain: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{ARBITRON_API}/v1/wallets/telegram:{user_id}/address/{chain}",
            headers={"Authorization": f"Bearer {API_KEY}"}
        )
        data = response.json()
        return data["address"]
...
async def handle_webhook(request: Request):
    """Обработчик webhook от Arbitron."""
...
async def balance_command(message: types.Message):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"{ARBITRON_API}/v1/wallets/telegram:{user_id}/balances",
            headers={"Authorization": f"Bearer {API_KEY}"}
        )
...
# ✅ Хорошо
import os
API_KEY = os.environ["ARBITRON_API_KEY"]
```
...
ARBITRON_API = os.environ["ARBITRON_API_URL"]
API_KEY = os.environ["ARBITRON_API_KEY"]
...
- Telegram: @arbitron_support
- Email: support@arbitron.io
- Документация: https://docs.arbitron.io
